...
void flandmark_write_model(const char* filename, FLANDMARK_Model* model) {
	...
	fprintf(fout, " %d %d ", model->data.options.bw[0],
			model->data.options.bw[1]);
	fprintf(fout, " %d %d ", model->data.options.bw_margin[0],
			model->data.options.bw_margin[1]);
	...
	for (int idx = 0; idx < model->data.options.M; ++idx) {
		fprintf(fout, " %d %d ", model->data.lbp[idx].WINS_ROWS,
				model->data.lbp[idx].WINS_COLS);
	}
	for (int idx = 0; idx < 3; ++idx) {
		fprintf(fout, " %d %d ", model->data.options.PSIG_ROWS[idx],
				model->data.options.PSIG_COLS[idx]);
		printf(
				"model->data.options.PSIG_ROWS[%d] = %d; model->data.options.PSIG_COLS[%d] = %d; \n",
				idx, model->data.options.PSIG_ROWS[idx], idx,
				model->data.options.PSIG_COLS[idx]);
	}
	...
	if (fwrite(model->data.mapTable, model->data.options.M * 4 * sizeof(int), 1,
			fout) != 1) {
		fclose(fout);
		printf("Error writing file %s\n", filename);
		exit(1);
	...
	for (int psigs_idx = 0; psigs_idx < 3; ++psigs_idx) {
		...
		tsize = model->data.options.PSIG_ROWS[psigs_idx]
				* model->data.options.PSIG_COLS[psigs_idx];
		...
		for (int idx = 0; idx < tsize; ++idx) {
			...
			if (fwrite(PsiGi[idx].disp, tmp_tsize * sizeof(int), 1, fout)
					!= 1) {
				fclose(fout);
				printf("Error writing file %s\n", filename);
				exit(1);
...
FLANDMARK_Model * flandmark_init(const char* filename) {
	...
	if (fscanf(fin, " %d %d ", &tst->data.options.bw[0],
			&tst->data.options.bw[1]) < 2) {
		return 0;
	}

	if (fscanf(fin, " %d %d ", &tst->data.options.bw_margin[0],
			&tst->data.options.bw_margin[1]) < 2) {
		return 0;
	}
	...
	if (fscanf(fin, " %d %d ", &tst->data.imSize[0], &tst->data.imSize[1])
			< 2) {
		return 0;
	}
	...
	for (int idx = 0; idx < M; ++idx) {
		if (fscanf(fin, " %d %d ", &tst->data.lbp[idx].WINS_ROWS,
				&tst->data.lbp[idx].WINS_COLS) < 2) {
			return 0;
		}
	}

	for (int idx = 0; idx < 3; ++idx) {
		if (fscanf(fin, " %d %d ", &tst->data.options.PSIG_ROWS[idx],
				&tst->data.options.PSIG_COLS[idx]) < 2) {
			return 0;
		}
	}
	...
	for (int idx = 0; idx < M; ++idx) {
		...
		if (fread(tst->data.lbp[idx].wins, tsize * sizeof(uint32_t), 1, fin)
				!= 1) {
			printf("Error reading file %s\n", filename);
			return 0;
			//exit(1);
	...
	FLANDMARK_PSIG * PsiGi = NULL;
	for (int psigs_idx = 0; psigs_idx < 3; ++psigs_idx) {
		tsize = tst->data.options.PSIG_ROWS[psigs_idx]
				* tst->data.options.PSIG_COLS[psigs_idx];

		switch (psigs_idx) {
		case 0:
			tst->data.options.PsiGS0 = (FLANDMARK_PSIG*) malloc(
					tsize * sizeof(FLANDMARK_PSIG));
			PsiGi = tst->data.options.PsiGS0;
			break;
		case 1:
			tst->data.options.PsiGS1 = (FLANDMARK_PSIG*) malloc(
					tsize * sizeof(FLANDMARK_PSIG));
			PsiGi = tst->data.options.PsiGS1;
			break;
		case 2:
			tst->data.options.PsiGS2 = (FLANDMARK_PSIG*) malloc(
					tsize * sizeof(FLANDMARK_PSIG));
	...
	tst->normalizedImageFrame = (uint8_t*) calloc(
			tst->data.options.bw[0] * tst->data.options.bw[1], sizeof(uint8_t));
...
EError_T flandmark_check_model(FLANDMARK_Model* model, FLANDMARK_Model* tst) {
	...
	if (model->data.options.bw[0] != tst->data.options.bw[0]
		...
		flag = false;
		printf("\n: %d ; %d", model->data.options.bw[0],
				tst->data.options.bw[0]);
		printf("\n: %d ; %d", model->data.options.bw[1],
				tst->data.options.bw[1]);
	}
	flag == true ? printf("passed. \n") : printf("NOT passed.\n");
	if (!flag) {
		return ERROR_BW;
	...
	if (model->data.options.bw_margin[0] != tst->data.options.bw_margin[0]
			|| model->data.options.bw_margin[1]
					!= tst->data.options.bw_margin[1]) {
		flag = false;
		printf("\n: %d ; %d", model->data.options.bw_margin[0],
				tst->data.options.bw_margin[0]);
		printf("\n: %d ; %d", model->data.options.bw_margin[1],
				tst->data.options.bw_margin[1]);
	}
	flag == true ? printf("passed. \n") : printf("NOT passed.\n");
	if (!flag) {
	...
	for (int idx = 0; idx < model->data.options.M; ++idx) {
		...
		for (int i = 0; i < 2; ++i) {
			if (tst->data.lbp[idx].winSize[i]
					!= model->data.lbp[idx].winSize[i]) {
				flag = false;
				printf("\n%d: %d ; %d", i, model->data.lbp[idx].winSize[i],
						tst->data.lbp[idx].winSize[i]);
				printf("Error.");
			}
		}
		...
		if (tst->data.lbp[idx].hop != model->data.lbp[idx].hop) {
			flag = false;
			printf("\n %d ; %d", model->data.lbp[idx].hop,
					tst->data.lbp[idx].hop);
			printf("Error.");
		}
		...
		for (int i = 0; i < tsize; ++i) {
			if (model->data.lbp[idx].wins[i] != tst->data.lbp[idx].wins[i]) {
				flag = false;
				printf("\n%d: %d ; %d", i, model->data.lbp[idx].wins[i],
						tst->data.lbp[idx].wins[i]);
				printf("Error.");
			}
		}
		flag == true ? printf("passed. \n") : printf("NOT passed.\n");
	...
	for (int i = 0; i < 4 * M; ++i) {
		if (model->data.options.S[i] != tst->data.options.S[i]) {
			flag = false;
			printf("\n%d: %d ; %d", i, model->data.options.S[i],
					tst->data.options.S[i]);
			printf("Error.");
		}
	}
	flag == true ? printf("passed. \n") : printf("NOT passed.\n");
	...
	for (int psig_idx = 0; psig_idx < 3; ++psig_idx) {
		...
		printf("options.PSIG_ROWS[%d]; options.PSIG_COLS[%d]... ", psig_idx,
				psig_idx);
		if (model->data.options.PSIG_ROWS[psig_idx]
				!= tst->data.options.PSIG_ROWS[psig_idx]
				|| model->data.options.PSIG_COLS[psig_idx]
						!= tst->data.options.PSIG_COLS[psig_idx]) {
			flag = false;
			printf("Error.");
		}
		flag == true ? printf("passed. \n") : printf("NOT passed.\n");
		...
		// disp
		flag = true;
		tsize = tst->data.options.PSIG_ROWS[psig_idx]
				* tst->data.options.PSIG_COLS[psig_idx];
		...
		for (int idx = 0; idx < tsize; ++idx) {
			...
			for (int i = 0; i < tmp_tsize; ++i) {
				if (PsiGi[idx].disp[i] != PsiGiTst[idx].disp[i]) {
					flag = false;
					printf("\nPsiGS%d[%d] =  %d; %d\n", psig_idx, idx,
							PsiGi[idx].disp[i], PsiGiTst[idx].disp[i]);
					printf("Error.");
				}
			}
		}
...
void flandmark_free(FLANDMARK_Model* model) {
	...
	for (int psig_idx = 0; psig_idx < 3; ++psig_idx) {
		...
		int tsize = model->data.options.PSIG_ROWS[psig_idx]
				* model->data.options.PSIG_COLS[psig_idx];
		for (int i = 0; i < tsize; ++i) {
			free(PsiGi[i].disp);
		}
...
void flandmark_get_psi_mat_sparse(FLANDMARK_PSI_SPARSE* Psi,
	...
	for (uint32_t i = 0; i < nData; ++i) {
		...
		liblbp_pyr_features_sparse(&Features[nDim * i], nDim, win, win_H,
				win_W);
	}
...
void flandmark_argmax(double *smax, FLANDMARK_Options *options,
		const int *mapTable, FLANDMARK_PSI_SPARSE *Psi_sparse, double **q,
		double **g) {
	uint8_t M = options->M;
	...
	for (int i = 0; i < q1_length; ++i) {
		...
		flandmark_maximize_gdotprod(
				//s2_maxs, s2_idxs,
				&s1[INDEX(0, i, 2)], (double*) &s1[INDEX(1, i, 2)], q[5], g[4],
				options->PsiGS1[INDEX(i, 0, options->PSIG_ROWS[1])].disp,
				options->PsiGS1[INDEX(i, 0, options->PSIG_ROWS[1])].COLS,
				tsize);
	...
	for (int i = 0; i < q2_length; ++i) {
		...
		flandmark_maximize_gdotprod(
				//s2_maxs, s2_idxs,
				&s2[INDEX(0, i, 2)], (double*) &s2[INDEX(1, i, 2)], q[6], g[5],
				options->PsiGS2[INDEX(i, 0, options->PSIG_ROWS[2])].disp,
				options->PsiGS2[INDEX(i, 0, options->PSIG_ROWS[2])].COLS,
				tsize);
	...
	int maxs0_idx = -1;
	double maxq10 = -FLT_MAX, maxq20 = -FLT_MAX, maxq30 = -FLT_MAX, maxq40 =
			-FLT_MAX, maxq70 = -FLT_MAX;
	double * s0 = (double *) calloc(M * q0_length, sizeof(double));
	for (int i = 0; i < q0_length; ++i) {
		// q10
		maxq10 = -FLT_MAX;
		flandmark_maximize_gdotprod(&maxq10, &s0[INDEX(1, i, M)], s1_maxs, g[0],
				options->PsiGS0[INDEX(i, 0, options->PSIG_ROWS[0])].disp,
				options->PsiGS0[INDEX(i, 0, options->PSIG_ROWS[0])].COLS,
				tsize);
		s0[INDEX(5, i, M)] = s1[INDEX(1, (int)s0[INDEX(1, i, M)], 2)];
		// q20
		maxq20 = -FLT_MAX;
		flandmark_maximize_gdotprod(&maxq20, &s0[INDEX(2, i, M)], s2_maxs, g[1],
				options->PsiGS0[INDEX(i, 1, options->PSIG_ROWS[0])].disp,
				options->PsiGS0[INDEX(i, 1, options->PSIG_ROWS[0])].COLS,
				tsize);
		s0[INDEX(6, i, M)] = s2[INDEX(1, (int)s0[INDEX(2, i, M)], 2)];
		// q30
		maxq30 = -FLT_MAX;
		flandmark_maximize_gdotprod(&maxq30, &s0[INDEX(3, i, M)], q[3], g[2],
				options->PsiGS0[INDEX(i, 2, options->PSIG_ROWS[0])].disp,
				options->PsiGS0[INDEX(i, 2, options->PSIG_ROWS[0])].COLS,
				tsize);
		// q40
		maxq40 = -FLT_MAX;
		flandmark_maximize_gdotprod(&maxq40, &s0[INDEX(4, i, M)], q[4], g[3],
				options->PsiGS0[INDEX(i, 3, options->PSIG_ROWS[0])].disp,
				options->PsiGS0[INDEX(i, 3, options->PSIG_ROWS[0])].COLS,
				tsize);
		// q70
		maxq70 = -FLT_MAX;
		flandmark_maximize_gdotprod(&maxq70, &s0[INDEX(7, i, M)], q[7], g[6],
				options->PsiGS0[INDEX(i, 4, options->PSIG_ROWS[0])].disp,
				options->PsiGS0[INDEX(i, 4, options->PSIG_ROWS[0])].COLS,
				tsize);
	...
	for (int i = 0; i < M; ++i) {
		...
		smax[INDEX(0, i, 2)] = float(
				COL(indices[i], rows) + optionsS[INDEX(0, i, 4)]);
		smax[INDEX(1, i, 2)] = float(
				ROW(indices[i], rows) + optionsS[INDEX(1, i, 4)]);
	}
	free(indices);
}
...
int flandmark_detect_base(uint8_t* face_image, FLANDMARK_Model* model,
	...
	// get PSI matrix
	FLANDMARK_PSI_SPARSE * Psi_sparse = (FLANDMARK_PSI_SPARSE*) malloc(
			M * sizeof(FLANDMARK_PSI_SPARSE));
	for (int idx = 0; idx < M; ++idx) {
	...
	for (int idx = 0; idx < M; ++idx) {
		...
		memcpy(q_temp, W + mapTable[INDEX(idx, 0, M)] - 1,
				tsize * sizeof(double));

		// sparse dot product <W_q, PSI_q>
		cols = Psi_sparse[idx].PSI_COLS;
		rows = Psi_sparse[idx].PSI_ROWS;
		...
		if (idx > 0) {
			...
			memcpy(g[idx - 1], W + mapTable[INDEX(idx, 2, M)] - 1,
					tsize * sizeof(double));
		}
	}

	// argmax
	flandmark_argmax(landmarks, &model->data.options, mapTable, Psi_sparse, q,
			g);
...
int flandmark_detect(IplImage *img, int *bbox, FLANDMARK_Model *model,
		double *landmarks, int *bw_margin) {
	int retval = 0;
	...
	retval = flandmark_get_normalized_image_frame(img, bbox, model->bb,
			model->normalizedImageFrame, model);
	if (retval) {
	...
	retval = flandmark_detect_base(model->normalizedImageFrame, model,
			landmarks);
	if (retval) {
	...
	model->sf[0] = (float) (model->bb[2] - model->bb[0])
			/ model->data.options.bw[0];
	model->sf[1] = (float) (model->bb[3] - model->bb[1])
			/ model->data.options.bw[1];
...
void flandmark_maximize_gdotprod(double * maximum, double * idx,
		const double * first, const double * second, const int * third,
		const int cols, const int tsize) {
	*maximum = -FLT_MAX;
	*idx = -1;
...
int flandmark_imcrop(IplImage *input, IplImage *output, const CvRect region) {
	if (input->width <= 0 || input->height <= 0 || region.width <= 0
			|| region.height <= 0) {
		return 1;
	}
	...
	if (output->width < region.width || output->height < region.height) {
		...
		output = cvCreateImage(cvSize(region.width, region.height),
				IPL_DEPTH_8U, 1);
	} else {
...
int flandmark_get_normalized_image_frame(IplImage *input, const int bbox[],
	...
	flag = bb[0] > 0 && bb[1] > 0 && bb[2] < input->width
			&& bb[3] < input->height && bbox[0] > 0 && bbox[1] > 0
			&& bbox[2] < input->width && bbox[3] < input->height;

	if (!flag) {
		return 1;
	}

	IplImage *croppedImage = cvCreateImage(cvSize(input->width, input->height),
			IPL_DEPTH_8U, 1);

	IplImage *resizedImage = cvCreateImage(
			cvSize(model->data.options.bw[0], model->data.options.bw[1]),
			IPL_DEPTH_8U, 1);

	// crop and resize image to normalized frame
	if (flandmark_imcrop(input, croppedImage,
			cvRect((int) bb[0], (int) bb[1], (int) bb[2] - (int) bb[0] + 1,
					(int) bb[3] - (int) bb[1] + 1))) {
		// something was bad
		return 1;
	}
	...
	for (int x = 0; x < model->data.options.bw[0]; ++x) {
		for (int y = 0; y < model->data.options.bw[1]; ++y) {
			face_img[INDEX(x, y, model->data.options.bw[1])] =
					(uint8_t) ((resizedImage->imageData
							+ resizedImage->widthStep * x)[y]);
		}
	}
